...
module.exports = class PocketBird extends Plugin {
	onload() {
		console.debug("Loading Pocket Bird version 2025.11.14.205...");
	 ...
	 */
	function isMobile() {
		if (OBSIDIAN_PLUGIN?.platform?.isMobile) {
			return true;
		}
		return false;
	}
	...
	class ObsidianContext extends Context {
		...
		getPath() {
			// @ts-expect-error
			const file = OBSIDIAN_PLUGIN.app.workspace.getActiveFile();
		...
		getActiveEditorElement() {
			// @ts-expect-error
			const activeLeaf = OBSIDIAN_PLUGIN.app.workspace.activeLeaf;
...
`;
	...
	Promise.all([
		...
		const settingsItems = [
			...
			new Separator(),
			new MenuItem("2025.11.14.205", () => { log("Thank you for using Pocket Bird! You are on version: 2025.11.14.205"); }, false),
		];
		...
		function insertFieldGuide() {
			...
			const generateDescription = (/** @type {string} */ speciesId) => {
				...
				spacer.classList.add("birb-field-guide-spacer");
